...
class CLRC : public QObject
{
    Q_OBJECT
public:
    explicit CLRC(QObject *parent = 0);

   #define CSlave_Address	0x28

   void I2CStartUp();
   void CLRC_write_byte(uint8_t reg, uint8_t val);
   unsigned char CLRC_read_byte(uint8_t reg);
   void CLRC_irq_wait(uint8_t irq0en, uint8_t irqlen);
   void CLRC_init();
   void CLRC_transmit_enable(bool enable);
   int CLRC_iso_14443A_reqa();
   int CLRC_iso_14443A_select();
   int CLRC_iso_14443A_rats();
   void mysleep(unsigned int delay);
   unsigned char CLRC_CONTROL_read_byte(uint8_t reg);
   ...
   void start_tranceive();
    void clrctoThread(QThread &clrcThread);
    QTimer *CLRCtimer1;
